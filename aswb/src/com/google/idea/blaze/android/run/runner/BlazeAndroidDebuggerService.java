...
public interface BlazeAndroidDebuggerService {
  ...
  /** Returns the standard debugger for non-native debugging. */
  @NotNull
   ...
   */
  void configureNativeDebugger(AutoAndroidDebuggerState state, BlazeAndroidDeployInfo deployInfo);
  ...
  class DefaultDebuggerService implements BlazeAndroidDebuggerService {
    ...
    public void configureNativeDebugger(
        AutoAndroidDebuggerState state, BlazeAndroidDeployInfo deployInfo) {
      ...
      StringBuilder symbolSearchPathsCommand = new StringBuilder();
      if (deployInfo != null && !deployInfo.getSymbolFiles().isEmpty()) {
        symbolSearchPathsCommand
            .append("settings append target.exec-search-paths ")
            .append(
